...
def calcZern(nz,coords,output=None):
    ...
    if 'typecode' in dir(output):
       output[:,]=Z.astype(output.typecode())
    elif 'dtype' in dir(output):
       output[:,]=Z.astype(output.dtype)
    else:
       raise ValueError("Cannot parse the output dtype")
    return output
...
class Zernike:
    ...
    def createZernikeCube(self,jmax):
        ...
        rGrid=util.tel.makeCircularGrid(self.npup,natype=self.natype,dosqrt=0)/self.npup**2*4
