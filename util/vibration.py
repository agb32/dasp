...
class Vibration:
    ...
    def __init__(self,pupil,telDiam,wfslam,tstep,freqList=[10.],angleList=[0.05],phaseList=[0.],dirnList=[0.]):
        """telDiam - in metres
        wfslam - in nm.
        npup - in pixels
        tstep - in seconds
    ...
    def angScaling(self,ang, phs, step, freq):
        scale=(ang*4.848e-6*self.telDiam)*(2*numpy.pi/self.wfslam*1e9)\
            *(2*self.npup**-1.0)*numpy.sin( phs + 2*numpy.pi*step*self.tstep*freq )
        return scale


    def vibrationMode(self,ang, phs, freq, rot):
        # \/ args:- angle, phase, frequency, and rotation (direction)
        #    as [arcsec],[rad],[Hz],[rad]

        scale=self.angScaling(ang,phs,self.itercnt,freq)
        t=numpy.cos(rot*numpy.pi/180.)*self.ttModes[0]+numpy.sin(rot*numpy.pi/180.)*self.ttModes[1]
        t*=scale
        return t

    def addVibration(self,out=None):
        if out is None:
            out=numpy.zeros((self.npup,self.npup),numpy.float32)
        for i in range(len(self.freqList)):
            out+=self.vibrationMode(self.angleList[i],self.phaseList[i],self.freqList[i],self.dirnList[i])
        self.itercnt+=1
        return out
