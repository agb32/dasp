...
def threadMain(ident):
   ...
   if shmem.acquireAndWait(semid) == 0:
      shmem.trigEvent(semid,1)#changed to include total number of threads.
   else:
...
semid=shmem.newsemid(nSems=4)
shmem.initSemaphore(semid, 0, 1)
shmem.initSemaphore(semid, 1, 1)
shmem.initSemaphore(semid, 2, 0)
shmem.initSemaphore(semid, 3, 0)
tlist=[]
