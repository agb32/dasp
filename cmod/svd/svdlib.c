...
DMat svdNewDMatWithData(int rows, int cols,double *data) {
  int i;
  DMat D = (DMat) malloc(sizeof(struct dmat));
  if (!D) {perror("svdNewDMat"); return NULL;}
  D->rows = rows;
  ...
  D->value[0] = data;//(double *) calloc(rows * cols, sizeof(double));
  if (!D->value[0]) {SAFE_FREE(D->value); SAFE_FREE(D); return NULL;}
...
DMat svdNewDMatWithDataFloat(int rows, int cols,float *data) {
  int i;
  DMat D = (DMat) malloc(sizeof(struct dmat));
  if (!D) {perror("svdNewDMat"); return NULL;}
  D->rows = rows;
  D->cols = cols;
  D->value=NULL;
  D->valuef = (float **) malloc(rows * sizeof(float *));
  if (!D->valuef) {SAFE_FREE(D); return NULL;}

  D->valuef[0] = data;//(double *) calloc(rows * cols, sizeof(double));
  if (!D->valuef[0]) {SAFE_FREE(D->valuef); SAFE_FREE(D); return NULL;}

  for (i = 1; i < rows; i++) D->valuef[i] = D->valuef[i-1] + cols;
  return D;
}

DMat svdNewDMatWithNoData(int rows, int cols) {
  DMat D = (DMat) malloc(sizeof(struct dmat));
  if (!D) {perror("svdNewDMat"); return NULL;}
  D->rows = rows;
  D->cols = cols;
...
SMat svdNewSMat(int rows, int cols, unsigned int vals) {
  ...
  S->cols = cols;
  S->vals = vals;
  S->pointr = svd_uintArray(cols + 1, TRUE, "svdNewSMat: pointr");
  if (!S->pointr) {svdFreeSMat(S); return NULL;}
  S->rowind = svd_uintArray(vals, FALSE, "svdNewSMat: rowind");
...
SMat svdNewSMatFloat(int rows, int cols, unsigned int vals) {
  ...
  S->rowind = svd_uintArray(vals, FALSE, "svdNewSMat: rowind");
  if (!S->rowind) {svdFreeSMat(S); return NULL;}
  S->valuef  = svd_floatArray(vals, FALSE, "svdNewSMat: valuef");
  if (!S->valuef)  {svdFreeSMat(S); return NULL;}
  S->value=NULL;
  return S;
}
...
DMat svdConvertStoD(SMat S) {
  ...
  if(S->value!=NULL){
    ...
    for (i = 0, c = 0; i < S->vals; i++) {
      while (S->pointr[c + 1] <= i) c++;
      D->value[S->rowind[i]][c] = (double)S->valuef[i];
    }
  }
  return D;
}
...
SMat svdTransposeS(SMat S) {
  ...
  SMat N;
  if(S->value!=NULL){
    N = svdNewSMat(S->cols, S->rows, S->vals);
  }else{//a float version...
    N=svdNewSMatFloat(S->cols,S->rows,S->vals);
  }
...
void svdWriteDenseArrayFloat(float *a, int n, char *filename, char binary) {
  int i;
  FILE *file = svd_writeFile(filename, FALSE);
  if (!file) 
    return svd_error("svdWriteDenseArray: failed to write %s", filename);
...
static void svdWriteSparseTextHBFile(SMat S, FILE *file) {
  ...
  fprintf(file, "%-14s%14u%14u%14u%14d\n", "rra", S->rows, S->cols,
          S->vals, 0);
  fprintf(file, "%16s%16s%16s%16s\n", "(8i)", "(8i)", "(8e)", "(8e)");

  for (i = 0; i <= S->cols; i++)
    fprintf(file, "%u%s", S->pointr[i] + 1, (((i+1) % 8) == 0) ? "\n" : " ");
  fprintf(file, "\n");
  for (i = 0; i < S->vals; i++)
    fprintf(file, "%u%s", S->rowind[i] + 1, (((i+1) % 8) == 0) ? "\n" : " ");
  fprintf(file, "\n");
  for (i = 0; i < S->vals; i++)
...
static SMat svdLoadSparseTextFile(FILE *file) {
  ...
  for (c = 0, v = 0; c < cols; c++) {
    ...
    S->pointr[c] = v;
    for (i = 0; i < n; i++, v++) {
      if (fscanf(file, " %u %lf", S->rowind + v, S->value + v) != 2) {
        svd_error("svdLoadSparseTextFile: bad file format");
        return NULL;
      }
    }
