...
static PyObject* ludecomp(PyObject *self,PyObject *args){
  ...
  if(Aarray->descr->type_num==NPY_FLOAT){
    clapack_sgetrf(CblasRowMajor,m,n,(float*)Aarray->data,m,(ATLAS_INT*)ipiv->data);//,&info);
  }else{
    clapack_dgetrf(CblasRowMajor,m,n,(double*)Aarray->data,m,(ATLAS_INT*)ipiv->data);//,&info);
  }
...
static PyObject *luinv(PyObject *self,PyObject *args){
  ...
  Py_BEGIN_ALLOW_THREADS;
  if(Aarray->descr->type_num==NPY_FLOAT){
    clapack_sgetri(CblasRowMajor,n,(float*)Aarray->data,n,(ATLAS_INT*)ipiv->data);//,&info);
  }else{
    clapack_dgetri(CblasRowMajor,n,(double*)Aarray->data,n,(ATLAS_INT*)ipiv->data);//,&info);
...
{
  PyObject *m;
  PyImport_AddModule("atlas");
  m=Py_InitModule("atlas", atlasMethods);
  import_array();
  atlasError = PyErr_NewException("atlas.error", NULL, NULL);
  Py_INCREF(atlasError);
  PyModule_AddObject(m, "error", atlasError);
}
