...
*/
/*
Python to perform SVD using intel MKL lapack.

Routine to perform SVD of a matrix using lapack.  Hopefully, similar to the scalapack version - I have a hunch that intel mkl may be multi-threaded, so may be faster than scalapack on a single computer (eg 8 core mac).
...
static PyObject* ludecomp(PyObject *self,PyObject *args){
  ...
  if(sizeof(MKL_INT)!=ipiv->descr->elsize || ipiv->descr->kind!='i'){
  ...
  if(Aarray->descr->type_num==NPY_FLOAT){
    sgetrf(&m,&n,(float*)Aarray->data,&m,(MKL_INT*)ipiv->data,&info);
  }else{
    dgetrf(&m,&n,(double*)Aarray->data,&m,(MKL_INT*)ipiv->data,&info);
  }
...
static PyObject *luinv(PyObject *self,PyObject *args){
  ...
  Py_BEGIN_ALLOW_THREADS;
  if(Aarray->descr->type_num==NPY_FLOAT){
    sgetri(&n,(float*)Aarray->data,&n,(MKL_INT*)ipiv->data,(float*)workarr,&lwork,&info);
  }else{
    dgetri(&n,(double*)Aarray->data,&n,(MKL_INT*)ipiv->data,(double*)workarr,&lwork,&info);
  ...
  if(checkWorkSize){
    if(Aarray->descr->type_num==NPY_FLOAT){
      lwork=(MKL_INT)(*((float*)workarr));
    }else{
      lwork=(MKL_INT)(*((double*)workarr));
    }
    free(workarr);
  }else{
    lwork=0;
...
static PyObject* svd(PyObject *self,PyObject *args){
  ...
  if(checkWorkSize==0){
    if(usesdd){
      if((iwork=(MKL_INT*)malloc(sizeof(MKL_INT)*8*n))==NULL){
...
static PyObject* gemm(PyObject *self,PyObject *args){
  ...
  ldc=(MKL_INT)Carray->strides[1]/wsize;
...
{
  PyObject *m;
  PyImport_AddModule("mkl");
  m=Py_InitModule("mkl", mklMethods);
  import_array();
  mklError = PyErr_NewException("mkl.error", NULL, NULL);
  Py_INCREF(mklError);
  PyModule_AddObject(m, "error", mklError);
}
