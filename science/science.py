...
#        self.pix_scale=self.L_D*float(self.npup)/float(nfft) ##pixel scale (arcsec/pixel in the science image)
        ...
        ##parameters related to the writing of simulation data on disk
#        self.write_to_file=config.getVal("write_to_file")
#        self.filename=config.getVal("simFilename")

        ##name of the FITS file to store the PSF
#        self.fitsFilename=config.getVal("fitsFilename")

        ##I (FA) have removed the tilt addition code, as we want the central intensity in one pixel and not spread on the 4 central pixels
        ##in order to compute precisely Strehl Ratio, ensquared and encircled energy, etc...

        ##Allocation of the memory for the arrays used to compute the FFT (done at the beginning so that allocation is made only 1 time => gain in execution time)
#        self.pupilAmplitude=Numeric.zeros((nfft,nfft),Numeric.Complex64) ##Complex amplitude in the pupil
        #self.focusAmplitude=Numeric.zeros((nfft,nfft),Numeric.Complex64) ##Complex amplitude a the focus = FFT of pupil complex amplitude function
#        self.focusAmplitude=self.pupilAmplitude#agb fix to save memory...
#        self.fftPlan= nrfft.fftw_plan_dft_2d(nfft,nfft,self.pupilAmplitude,self.focusAmplitude,nrfft.FFTW_FORWARD,nrfft.FFTW_MEASURE) ##FFTW plan
#        self.tempImg=Numeric.zeros((nfft,nfft),Numeric.Float64)#[]## Square modulus of the FFT of pupilAmplitude
        
        ##Allocation of the memory for the short and long exp PSF
#        self.longexp_img=Numeric.zeros((nfft,nfft),Numeric.Float64)# Long exposure image array
#        self.inst_img=Numeric.zeros((nfft,nfft),Numeric.Float64)#[]# Instantaneous image - dummy size since copied later

        ##Computes the diffraction limited PSF
#        self.computeDiffPSF() ##creates the diffPSF array

        ##Numarray array used to write the DL PSF and the AO-corrected PSF every scinSamp iterations
##         if self.fitsFilename!=None:
##             self.psfWrittenToDisk=Numeric.zeros((2,nfft,nfft),self.diffPSF.typecode())#numarray.array(shape=(nfft,nfft,2),typecode=self.diffPSF.typecode())
##             self.psfWrittenToDisk[0]=self.diffPSF
##         else:
##             self.psfWrittenToDisk=None

        ##Array storing the input phase
#        if self.atmosPhaseType=="phaseonly":
#            self.phs=Numeric.zeros((npup,npup),Numeric.Float64)
#        elif self.atmosPhaseType=="phaseamp":#data[0]==phase, data[1]==amplitude
#            self.phs=Numeric.zeros((2,npup,npup),Numeric.Float64)
#        elif self.atmosPhaseType=="realimag":
#            self.phs=Numeric.zeros((npup,npup),Numeric.Complex64)
        
        #### The next function initialises the arrays required to
        #### fastly compute the radial profile of the PSF as well as the encircled energy
        #### This is done with a better spatial sampling than before (non integer pixel values)
#        self.initRadialProfile()
#        self.rad=self.rRad*self.pix_scale ##radius (in arcsec) giving the horizontal axis for radial and encircled energy profiles

        #### We compute the radial profile of the diffraction limited PSF
#        self.dlPsfRadialProfile=self.computeRadialProfileAndEncircledEnergy(self.diffPSF)[0,:,]

        #### Central intensity (used to compute Strehl ratio)
#        self.diffn_core_en=self.diffPSF[nfft/2,nfft/2] ##Coordinates consistent with FFT coordinates definitions

        #### The next function initialises the arrays required to
        #### fastly compute the ensquared energy profile of the PSF
#        self.initEnsquaredEnergyProfile()

        #### We compute the radial profile of the diffraction limited PSF
#        self.dlPsfEnsquaredProfile=self.computeEnsquaredEnergy(self.diffPSF)

        #### Allocation of the arrays to store ensquared energy, 
#        self.inbox=Numeric.zeros(nfft/2,Numeric.Float64)# In box energy
#        self.wid=self.rSquare*self.pix_scale #Square aperture size in arcsec. X Axis of inbox array, used for plotting		

        #### Refresh rate for the computation of the SR, FWHM, etc...
#        self.scinSamp=config.getVal("scinSamp")
#        self.isamp=0 ##Counter used for the computation of scientific values. If isamp=scinSamp then we compute the scientific values

        #### Lists to be plotted
#        self.scienceListsSize=config.getVal("hist_list_size")
#        self.strehl=0. ## Strehl ratio
#        self.strehl_hist=[] ## History of Strehl ratio (for plotting)
#        self.d50=0. ## diameter of the circular aperture with 50% of the energy
#        self.d50_hist=[] ##History of d50
#        self.fwhm=0. ##FWHM
#        self.fwhm_hist=[] ##History of fwhm
#        self.inbox_2=0. ##This parameter is the diameter of the circular aperture with 20% of the energy
#        self.inbox_hist=[] ##History of inbox_2
        #self.scienceListsSize=config.getVal("hist_list_size") ## Number of elements in the history lists, to have lists with a reasonable size
        
        ## Dictionary with arrays to plot
#        self.plot_data={}

        ## Dictionary with the science values (FWHM, d50, Strehl, inbox)
#        self.dictScience={}

        ## Number of PSF currently used for integration
#        self.n_integn=0
#        self.longExpPSF=Numeric.zeros((nfft,nfft),Numeric.Float64)# Long exposure image array
        #timestep stuff
        #self.wfs_texp=0
        #self.wfs_lat=config.getVal("wfs_lat")
        #self.wfs_int=config.getVal("wfs_int")
...
##             this.pupilAmplitude=cmod.utils.arrayFromArray(self.pupilAmplitudeMem,(this.nfft,this.nfft),Numeric.Complex64)
##             this.focusAmplitude=this.pupilAmplitude#save memory
##             this.fftPlan= nrfft.fftw_plan_dft_2d(this.nfft,this.nfft,this.pupilAmplitude,this.focusAmplitude,nrfft.FFTW_FORWARD,nrfft.FFTW_MEASURE) ##FFTW plan
##             this.tempImg=cmod.utils.arrayFromArray(self.tempImgMem,(this.nfft,this.nfft),Numeric.Float64)
##             this.inst_img=cmod.utils.arrayFromArray(self.inst_imgMem,(this.nfft,this.nfft),Numeric.Float64)
##             this.longExpPSF=cmod.utils.arrayFromArray(self.longExpPSFMem,(this.nfft,this.nfft),Numeric.Float64)
##             self.atmosPhaseType=this.atmosPhaseType
##             self.npup=this.npup
##             self.pupilAmplitude=this.pupilAmplitude
##             self.pup=this.pup
##             self.fftPlan=this.fftPlan
##             self.focusAmplitude=this.focusAmplitude
##             self.tempImg=this.tempImg
##             self.inst_img=this.inst_img
##             self.keepDiffPsf=this.keepDiffPsf
##             this.diffPSF=self.computeDiffPSF()
##             # We compute the radial profile of the diffraction limited PSF
##             this.diffPsfRadialProfile=self.computeRadialProfileAndEncircledEnergy(this.diffPSF)[0,:,]
##             if this.diffPsfFilename!=None:#save the diffraction limited PSF.
##                 util.FITS.Write(this.diffPSF,this.diffPsfFilename)
##             if this.atmosPhaseType=="phaseonly":
##                 this.phs=cmod.utils.arrayFromArray(self.phsMem,(npup,npup),Numeric.Float64)
##             elif this.atmosPhaseType=="phaseamp":#data[0]==phase,data[1]==amp
##                 this.phs=cmod.utils.arrayFromArray(self.phsMem,(2,npup,npup),Numeric.Float64)
##             elif this.atmosPhaseType=="realimag":
##                 this.phs=cmod.utils.arrayFromArray(self.phsMem,(npup,npup),Numeric.Complex64)
##             self.nnRad=this.nnRad
##             self.xRad=this.xRad
##             self.tabNbPointsRad=this.tabNbPointsRad
##             this.dlPsfRadialProfile=self.computeRadialProfileAndEncircledEnergy(this.diffPSF)[0,:,]
##             #central intensity (used to compute strehl ratio)
##             this.diffn_core_en=this.diffPSF[nfft/2,nfft/2]
##             # We compute the radial profile of the diffraction limited PSF
##             self.nnSquare=this.nnSquare
##             self.xSquare=this.xSquare
            
##             this.diffPsfEnsquaredProfile=self.computeEnsquaredEnergy(this.diffPSF)
